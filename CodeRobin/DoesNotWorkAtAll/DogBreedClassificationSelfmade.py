...
files = listdir("images/Images/allImages/")     #creates list with all imges of breed i
for n in range(len(listdir("images/Images/allImages/"))):
    ...
    img = Image.open("images/Images/allImages/" + j).convert('RGB')  # load example image from images
    ...
    if len(train_data_list) >= batchsize:  # sets batchsize
        ...
        print('Loaded batch ', len(train_data), ' of ', int(len(listdir(
            'images/Images/allImages/')) / batchsize))
        percentage = (len(train_data) / int(len(listdir('images/Images/allImages/')) / batchsize)) * 100
...
def test():
    model.eval()
    files = listdir('Classification/images/Testimages/')
